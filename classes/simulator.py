...
class Simulator:
    ...
    def modulated_sine(self, *, Vpp: float = 0.26, phase_mod_cycles: float = 2.9, phase_mod_frequency: float = 150,
                       ...
                       core_index: float = 1.52, ac_voltage: float = 1200, eff_distance: float = 41.1e-6, field_adjustment_factor: float = 1.7,
        ...
        phase_change = ((2 * np.pi * fiber_length * chi2 * ac_voltage) / (wavelength * core_index * field_adjustment_factor * eff_distance)) * np.cos(2 * np.pi * ac_frequency * (self.time - t_offset))

        signal = (Vpp / 2) * (1 + np.cos((2 * np.pi * phase_mod_cycles * phase_mod_frequency * (self.time - t_offset)) - (phase_change / 2)))
        return signal
    
    def noisy_modulated_sine(self, *, Vpp: float = 1, phase_mod_cycles: float = 2.5, phase_mod_frequency: float = 120,
                       fiber_length: float = 0.3, wavelength: float = 1550e-9, chi2: float = 0.15e-12,
                       core_index: float = 1.45, ac_voltage: float = 240, eff_distance: float = 41.1e-6, field_adjustment_factor: float = 1.773777,
                       ac_frequency: float = 3000, t_offset: float = 0, SNR: float) -> NDArray[float64]:
        """Modulated sinusoidal signal generated by the interferometer during the chi-2 measurement experiment.
        ...
        signal = self.modulated_sine(Vpp=Vpp, phase_mod_cycles=phase_mod_cycles, phase_mod_frequency=phase_mod_frequency,
                                     ...
                                     ac_voltage=ac_voltage, eff_distance=eff_distance, field_adjustment_factor=field_adjustment_factor,
                                     ac_frequency=ac_frequency, t_offset=t_offset)
